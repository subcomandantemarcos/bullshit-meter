import { PrivateNewsDetailsPageComponent } from './details.component';
import { PrivateNewsDetailsPageEffects, privateNewsDetailsPageReducer } from './shared/store';
import { AppState } from '@shared/store/state';
import { EffectsModule } from '@ngrx/effects';
import { Store, StoreModule } from '@ngrx/store';
import { TestBed } from '@angular/core/testing';
import { render, RenderResult } from '@testing-library/angular';
import { configuration } from '@configurations';
import { TranslateTestingModule } from 'ngx-translate-testing';
import { PrivateNewsDetailsPageFacade } from './details.facade';

describe('PrivateNewsDetailsPageComponent', () => {
  let component: RenderResult<PrivateNewsDetailsPageComponent>;
  let componentInstance: PrivateNewsDetailsPageComponent;
  let store: Store<AppState>;

  const translation = require(`../../../../assets/i18n/${configuration.language.default}.json`);

  beforeEach(async () => {
    component = await render(PrivateNewsDetailsPageComponent, {
      imports: [
        TranslateTestingModule.withTranslations(configuration.language.default, translation),
        StoreModule.forRoot({}, {
          runtimeChecks: {
            strictStateImmutability: true,
            strictActionImmutability: true
          }
        }),
        StoreModule.forFeature('privateNewsDetailsPage', privateNewsDetailsPageReducer),
        EffectsModule.forRoot([]),
        EffectsModule.forFeature([PrivateNewsDetailsPageEffects])
      ],
      declarations: [
        PrivateNewsDetailsPageComponent
      ],
      routes: [],
      providers: [PrivateNewsDetailsPageFacade]
    });

    componentInstance = component.fixture.componentInstance;
    store = TestBed.inject(Store);
  });

  it('should create', async () => {
    expect(component).toBeDefined();
  });
});

